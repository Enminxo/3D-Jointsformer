...
class SkeletonTransformer(nn.Module):
    ...
    def __init__(self, in_joints=21, out_joints=30):
        super().__init__()
        self.in_joints = in_joints
        self.out_joints = out_joints

        # self.trans_mat = nn.Parameter(torch.empty(in_joints, out_joints))
        self.trans_mat = nn.Parameter(torch.empty(21, 30))
        nn.init.orthogonal_(self.trans_mat)

    def forward(self, x):
        n, c, t, v = x.size()  # v = num_joints = 21
        x = x.view(-1, v)  # keep the rest of the dim the same, operating only on the joints
        y = torch.matmul(x, self.trans_mat)  # learns the joints
        y = y.view(n, c, t, -1)  # returns what
        return y
